      ...
      for (let i = 0; i < playerArray.length; i++) {
        if (
          ...
          if (i == 0) {
            ...
            // io.to(playerArray[1].socketID).emit('announce', 'you win')
            // io.to(playerArray[0].socketID).emit('announce', 'you lose')
          } else if (i == 1) {
            setTimeout(() => {
              io.emit('announce' , 'Red win') 
            }, 100);
            
            // io.to(playerArray[0].socketID).emit('announce', 'you win')
            // io.to(playerArray[1].socketID).emit('announce', 'you lose')
          }
