...
const BulletDown = document.getElementById('jenBulletDown');
const BulletUp = document.getElementById('jenBulletUp');
const BulletLeft = document.getElementById('jenBulletLeft');
const BulletRight = document.getElementById('jenBulletRight');


const imageWater = document.getElementById('water');
const imagePlant = document.getElementById('plant');
const imageBrick = document.getElementById('brick')
const imageSteel = document.getElementById('steel')
    ...
    if (bulletArrayInClient.length != 0) {
        for (let i = 0; i < bulletArrayInClient.length; i++) {
            ...
            switch (bulletToBeDrew.direction) {
                case "pointUp":
                    ctx.drawImage(BulletUp, bulletToBeDrew.x - 5, bulletToBeDrew.y, bulletToBeDrew.w, bulletToBeDrew.h)
                    break
                case "pointDown":
                    ctx.drawImage(BulletDown, bulletToBeDrew.x - 5, bulletToBeDrew.y - 10, bulletToBeDrew.w, bulletToBeDrew.h)
                    break
                case "pointLeft":
                    ctx.drawImage(BulletLeft, bulletToBeDrew.x, bulletToBeDrew.y - 5, bulletToBeDrew.w, bulletToBeDrew.h)
                    break
                case "pointRight":
                    ctx.drawImage(BulletRight, bulletToBeDrew.x - 10, bulletToBeDrew.y - 5, bulletToBeDrew.w, bulletToBeDrew.h)
                    break
                default:
            }
        }
...
function drawBrick() {
    for (let i = 0; i < brickArrayInClient.length; i++) {
        ...
        ctx.drawImage(imageBrick, brickToBeDrew.x_Pos_Tile, brickToBeDrew.y_Pos_Tile, brickToBeDrew.w, brickToBeDrew.h)
    }
}
...
function drawSteel() {
    for (let i = 0; i < steelArrayInClient.length; i++) {
        ...
        ctx.drawImage(imageSteel, steelToBeDrew.x_Pos_Tile, steelToBeDrew.y_Pos_Tile, steelToBeDrew.w, steelToBeDrew.h)
    }
}
